import{r as i,j as a}from"./index-DQroLkoY.js";const l=`
...
`,n=[{label:"Smart Persistence",clean:.0948,dust:.1843,penalty:94.5},{label:"Transformer — No AOD",clean:.0097,dust:.0331,penalty:242.8},{label:"Transformer — AOD",clean:.0105,dust:.0372,penalty:256},{label:"XGBoost — No AOD",clean:.0246,dust:.0618,penalty:151.2},{label:"XGBoost — AOD",clean:.0277,dust:.0634,penalty:128.9},{label:"MLP — No AOD",clean:.0605,dust:.0837,penalty:38.3},{label:"MLP — AOD",clean:.061,dust:.0928,penalty:52.1},{label:"RF — No AOD",clean:.0734,dust:.1184,penalty:61.3},{label:"RF — AOD",clean:.0752,dust:.1204,penalty:60.1}];function m(){return i.useEffect(()=>{const r=document.getElementById("dustBars");r&&r.childElementCount===0&&n.forEach(e=>{const s=e.penalty>200?"#E07060":e.penalty>100?"#D4A060":"#7BB88C";r.innerHTML+=`
          <div class="dust-bar-row">
            <div class="dust-bar-label">${e.label}</div>
            <div class="dust-bar-tracks">
              <div class="dust-bar-track"><div class="dust-bar-clean" style="width:${(e.clean/.2*100).toFixed(1)}%"></div></div>
              <div class="dust-bar-track"><div class="dust-bar-dust-fill" style="width:${(e.dust/.2*100).toFixed(1)}%"></div></div>
            </div>
            <div class="dust-penalty-label" style="color:${s}">${e.penalty.toFixed(0)}%</div>
          </div>`});const o=new IntersectionObserver(t=>{t.forEach(e=>{e.isIntersecting&&(e.target.classList.add("visible"),o.unobserve(e.target))})},{threshold:.08});return document.querySelectorAll(".home-root .fade-up").forEach(t=>o.observe(t)),()=>o.disconnect()},[]),a.jsxs(a.Fragment,{children:[a.jsx("style",{dangerouslySetInnerHTML:{__html:l}}),a.jsx("div",{className:"home-root",dangerouslySetInnerHTML:{__html:d}})]})}export{m as component};
